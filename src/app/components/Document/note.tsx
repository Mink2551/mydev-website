...
const Note: React.FC<NoteProps> = ({ noteId }) => {
    ...
    useEffect(() => {
      // Your existing code for fetching note data
    }, [noteId, db, mode]);
    ...
    const handleSave = async () => {
      ...
      try {
        await update(noteRef, { content: newContent, lastEdited: timestamp });
        await update(ref(db, `files/${noteId}`), { name: fileName });
